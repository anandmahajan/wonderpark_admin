      ...
      <div class="component-section mt-2 mb-4">
        <div class="row mb-3">
          ...
          <div class="col-md-9 text-md-right">
            <button type="button" class="btn btn-brand-01" @click="addAdmin()">
                ...
                <tr v-for="(item, i) in list" :key="item.id">
                  ...
                  <td>
                    <button
                      ...
                      @click="editAdmin(item.uuid)"
                    >
                      {{ $lang.messages.edit }}
                    </button>
                    <button
                      v-if="admin_id != item.id"
                      type="button"
...
export default {
  ...
  methods: {
    ...
    addAdmin() {
      ...
      this.$shareService.setMapValue("admin_search", this.filterObj.keyword);
  ...
  mounted() {
    ...
    if (this.$shareService.getMapValue("admin_search")) {
      this.filterObj.keyword = this.$shareService.getMapValue("admin_search");
      this.$shareService.deleteMapValue("admin_search");
    }

    var tabValue1 = this.$shareService.getMapValue("admin_page");

    setTimeout(() => {
      if (tabValue1 != null && tabValue1 != "") {
        this.getList(this.$shareService.getMapValue("admin_page"));
        this.$shareService.deleteMapValue("admin_page");
      } else {
        this.getList(1);
      }
    }, 200);
